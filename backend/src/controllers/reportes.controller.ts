...
function getDateRange(req: Request): { fechaDesde: string; fechaHasta: string } {
  ...
  const hasta  = (req.query['fechaHasta'] as string | undefined) ?? ahora.toISOString().slice(0, 10)
  const treintaDias = new Date(ahora)
  treintaDias.setDate(treintaDias.getDate() - 30)
  const desde  = (req.query['fechaDesde'] as string | undefined) ?? treintaDias.toISOString().slice(0, 10)
